                ...
                {blogCategories.map((item, index) => (
                    <div key={index} className='relative'>
                        <button onClick={() => setMenu(item)} className={`cursor-pointer text-gray-500 ${menu === item && 'text-white px-4 pt-0.5'}`}>
                            {item}
                            {menu === item && (
                                <motion.div layoutId='underline' transition={{ type: 'string', stiffness: 500, damping: 30 }} className="absolute left-0 right-0 top-0 h-7 -z-10 bg-primary rounded-full"></motion.div>
                            )}
                        </button>
                    </div>
                ))}
